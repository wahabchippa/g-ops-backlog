...
# 3. CSS (Original 100% + 3PL Table Styling)
st.markdown("""
<style>
...
.sidebar-toggle {
    ...
    border: 1px solid #444; border-radius: 8px; padding: 8px 16px; color: white; font-weight: 600; cursor: pointer;
}
...
# 4. DATA LOADING (Original + Fixed 3PL)
...
def load_data():
    ...
    ai_fleek_ids = set(ai_df['fleek_id'].astype(str).str.strip().tolist())
        ...
        try:
            ...
            for r in regions:
                ...
                for i, d in enumerate(dates):
                    ...
                    for j, v in enumerate(vals): totals[(i*5)+j] += v; html += f'<td>{int(v) if v > 0 else ""}</td>'
                html += '</tr>'
...
# 7. MAIN APP (Original 100% Layout)
try:
